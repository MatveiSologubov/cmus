...
static void fill_track_fopts_track_info(struct track_info *info) {
  ...
  fopt_set_int(&track_fopts[TF_ORIGINALYEAR], info->originaldate / 10000,
               info->originaldate <= 0);
  fopt_set_int(&track_fopts[TF_BITRATE], (int)(info->bitrate / 1000. + 0.5),
               info->bitrate == -1);
...
static void fill_track_fopts_artist(struct artist *artist) {
  const char *name =
      display_artist_sort_name ? artist_sort_name(artist) : artist->name;
...
const struct format_option *get_global_fopts(void) {
  ...
  buffer_fill =
      scale_to_percentage(player_info.buffer_fill, player_info.buffer_size);
...
static void print_tree(struct window *win, int row, struct iter *iter) {
  ...
  if (album) {
    ...
    format_print(&print_buffer, tree_win_w - 1, tree_win_artist_format,
                 track_fopts);
  }
...
static void print_track(struct window *win, int row, struct iter *iter) {
  ...
  if (track == (struct tree_track *)album) {
    ...
    len = format_print(&print_buffer, track_win_w, track_win_album_format,
                       track_fopts);
...
static void print_filter(struct window *win, int row, struct iter *iter) {
  ...
  snprintf(buf, sizeof(buf), "%c%c%c%-15s  %.235s", ch1, ch2, ch3, e->name,
           e_filter);
...
static void print_help(struct window *win, int row, struct iter *iter) {
  ...
  case HE_BOUND:
    snprintf(buf, sizeof(buf), " %-8s %-23s %s",
             key_context_names[e->binding->ctx], e->binding->key->name,
             e->binding->cmd);
    break;
  case HE_UNBOUND:
...
static void update_tree_window(void) {
  ...
  update_window(lib_tree_win, tree_win_x, 0, tree_win_w + 1, buf.buffer,
                print_tree);
}
...
static void update_editable_window(struct editable *e, const char *title,
                                   const char *filename) {
  static GBUF(buf);
  gbuf_clear(&buf);
  ...
  if (filename) {
    ...
    gbuf_addf(&buf, "%s %.256s - %d tracks", title, pretty_path(filename),
              e->nr_tracks);
  } else {
  ...
  gbuf_addf(&buf, " %s%s", sorted_names[e->shared->sort_str[0] != 0],
            e->shared->sort_str);
  ...
  if (progress_bar && player_info.ti) {
    ...
    if (duration && duration >= player_info.pos) {
      if (progress_bar == PROGRESS_BAR_LINE ||
        ...
        int end_offset =
            u_skip_chars(print_buffer.buffer + buf_index, &shuttle_len, true);
...
static void do_update_titleline(void) {
  ...
  if (player_info.ti) {
    ...
    if (use_alt_format && *window_title_alt_format) {
      format_print(&print_buffer, 0, window_title_alt_format, track_fopts);
    } else {
      format_print(&print_buffer, 0, window_title_format, track_fopts);
    }
...
int get_client_fd(void) { return client_fd; }
...
static int fill_status_program_track_info_args(char **argv, int i,
  ...
  static const char *keys[] = {"artist",     "albumartist",         "album",
...
static void handle_csi(void) {
  ...
  if (buf_n == 4) {
    ...
    if (buf[0] == '2' && buf[1] == '0' && (buf[2] == '0' || buf[2] == '1') &&
        buf[3] == '~') {
...
static void init_all(void) {
  ...
  if (resume_cmus) {
    resume_load();
    cmus_add(play_queue_append, play_queue_autosave_filename, FILE_TYPE_PL,
             JOB_TYPE_QUEUE, 0, NULL);
  } else {
    set_view(start_view);
  }
...
static void exit_all(void) {
  ...
  cmus_exit();
  if (resume_cmus)
    cmus_save(play_queue_for_each, play_queue_autosave_filename, NULL);
...
static const char *usage =
    ...
    "\n"
    "      --listen ADDR   listen on ADDR instead of $CMUS_SOCKET or "
